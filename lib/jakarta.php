<?php
include ("etc/support.php");
$nama = nama();
        $email = str_replace(" ", "", $nama) . mt_rand(100, 999);
        ulang:
        echo color("nevy","[?] Nomor Lu Cok : ");
        // $no = trim(fgets(STDIN));
        ...
        if(strpos($register, '"otp_token"')){
			...
			if(strpos($verif, '"access_token"')){
				...
				echo color("green","\n===========(REDEEM VOUCHER)===========");
				echo "\n".color("yellow","[!] Claim Voc 1");
				echo "\n".color("yellow","[!] Tunggu Bentar Napa Cok...");
				for($a=1;$a<=3;$a++){
					echo color("yellow",".");
					sleep(1);
				}
				...
				if(strpos($code1, 'You can use this promo now...')){
					...
					sleep(3);
					$goride = request('/go-promotions/v1/promotions/enrollments', $token, '{"promo_code":"COBAGOCARPAY"}');
					$message1 = fetch_value($goride,'"message":"','"');
					echo "\n".color("green","[+] Message: ".$message1);
							
					echo "\n".color("yellow","[!] Claim Voc 4");
					echo "\n".color("yellow","[!] Tunggu Bentar Napa Cok...");
					for($a=1;$a<=3;$a++){
						echo color("yellow",".");
						sleep(1);
				...
				echo color("red","[-] Code Mu Salah Coeg -_-");
				echo color("green", "\n =================================== \n\n");
				echo color("yellow","[!] Masukin Lagi \n");
				goto otp;
            }
		}else{
			echo color("red","[-] Udah Teregistrasi Cok !!!");
			echo color("green", "\n =================================== \n\n");
			echo color("yellow","[!] Pakai Nomor Baru Sue -_- \n");
			goto ulang;
        }
//	}
// echo change()."\n";
